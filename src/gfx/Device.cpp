...
auto gfx::Device::createSwapchain(Surface const& surface, const SurfaceConfiguration& config) -> Swapchain {
    auto capabilities = shared->adapter.getSurfaceCapabilitiesKHR(surface.shared->raw, shared->instance.dispatcher());
...
//    }
    ...
    swapchain_create_info.setSurface(surface.shared->raw);
    ...
    auto swapchain = shared->raii.raw.createSwapchainKHR(swapchain_create_info, nullptr, shared->raii.dispatcher);
    auto images = shared->raii.raw.getSwapchainImagesKHR(swapchain, shared->raii.dispatcher);
    ...
    for (size_t i = 0; i < images.size(); ++i) {
        ...
        drawables[i] = Drawable(swapchain, Texture(texture), uint32_t(i));
    }
    return Swapchain(std::make_shared<SwapchainShared>(*this, surface, swapchain, drawables));
}